...
int mode;
{
	...
	if(desmode != DES_STANDARD)	/* No permutations */
		return 0;
...
int crypt_de(char *buf, int size, int cont) {
	...
	while(1) {
		...
		if (cnt == size) {
			...
			/* corrupted format or bad key */
			if (work[DES_BLOCKS-1] > DES_BLOCKS || 
			    work[DES_BLOCKS-1] < 0)
				return -1;
			return cnt - work[DES_BLOCKS-1];
		}
	}
}
