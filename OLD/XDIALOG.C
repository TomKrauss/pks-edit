...
static void DlgInit(HWND hDlg, DIALPARS *dp)
{
	...
	if (!DoDlgInitPars(hDlg, dp, nPars)) {
		form_move(hDlg);
	}
}
...
#if 0
	if ((newstate = (IsDlgButtonChecked(hDlg, b1) == 0)) == 1)
		CheckDlgButton(hDlg, b2, 0);
	CheckDlgButton(hDlg, b1, newstate);
#endif
}
...
{
	int		idCtrl;
	char 	cbuf[2],numbuf[32],szBuff[128],szButton[28];
	int  	nNotify,nIndex,item,*ip,(*callback)();
	LONG 	c;
	DIALPARS *dp2;
	...
	switch(idCtrl) {
		...
		case IDD_PATH1SEL:
			...
			if (EdFsel(szBuff,numbuf,_fseltarget)) {
